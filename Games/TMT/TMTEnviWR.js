...
function showInfo(mapWR){
    ...
    for(wr of mapWR){
        if(wr[6] == "TMT"){
            ...
            let diff;
            if(map < 41){diff = "A";}
            else if(map < 81){diff = "B";}
            else if(map < 121){diff = "C";}
            else if(map < 161){diff = "D";}
            else {diff = "E";}
            if (EnviWrCount[diff]) {
                EnviWrCount[diff] += 1;
            } else {
    ...
    WrStatTMTWhite.innerHTML = "> <span class='WrCount'>" + EnviWrCount["A"] + "</span> WR Archived";

    let WrStatTMTGreen = document.getElementById("WrStatTMTGreen");
    WrStatTMTGreen.innerHTML = "> <span class='WrCount'>" + EnviWrCount["B"] + "</span> WR Archived";
    ...
    WrStatTMTBlack.innerHTML = "> <span class='WrCount'>" + EnviWrCount["E"] + "</span> WR Archived";
}
