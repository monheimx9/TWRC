...
function showInfo(mapWR, Nation, Flag, playerDB) {
    ...
    for (wr of mapWR) {
        if (wr[8] == "a") {
            ...
            if (tableID == wr[8]) {
                playerINFO = playerWrList.push(wr);
            }
        }
    }
    ...
    let gamesInfo_setup = "";
    for (elem of overallStats[1]) {
        gamesInfo_setup += "<span class='mapGameSpan'>" + elem + "</span>" + "/ ";
    }
    ...
    let envisInfo_setup = "";
    for (elem of overallStats[0]) {
        envisInfo_setup +=
            "<img src='../../assets/enviLogos/" +
            elem +
            ".png' style='height: 22px; padding-top: 5px;'>";
    }
    ...
    for (elem of overallStats[3][0]) {
        prolificMapInfo_setup += elem + " / ";
    }
    prolificMapInfo.innerHTML =
        prolificMapInfo_setup.slice(0, -3) + " · " + overallStats[3][1] + " WRs";

    /* prolificGameInfo */
    const prolificGameInfo = document.getElementById("prolificGameInfo");
    let prolificGameInfo_setup = "";
    for (elem of overallStats[5][0]) {
        prolificGameInfo_setup += "<span class='mapGameSpan'>" + elem + "</span>/ ";
    }
    prolificGameInfo.innerHTML =
        prolificGameInfo_setup.slice(0, -2) + " · " + overallStats[5][1] + " WRs";

    /* prolificEnviInfo */
    const prolificEnviInfo = document.getElementById("prolificEnviInfo");
    let prolificEnviInfo_setup = "";
    for (elem of overallStats[6][0]) {
        prolificEnviInfo_setup +=
            "<img src='../../assets/enviLogos/" +
            elem +
            ".png' style='height: 23px; vertical-align: bottom;'> / ";
    }
    prolificEnviInfo.innerHTML =
        prolificEnviInfo_setup.slice(0, -3) + " · " + overallStats[6][1] + " WRs";
    ...
    for (let i = 1; i < playerWrList.length; i++) {
        ...
        if (testdate[3] != "??/??/????") {
            firstWrInfo_setup = testdate;
            break;
        }
    }
    firstWrInfo.innerHTML =
        firstWrInfo_setup[1] + " · " + firstWrInfo_setup[2] + " · " + firstWrInfo_setup[3];

    /* latestWrInfo */
    const latestWrInfo = document.getElementById("latestWrInfo");
    latestWrInfo.innerHTML =
        playerWrList[0][1] + " · " + playerWrList[0][2] + " · " + playerWrList[0][3];
}
